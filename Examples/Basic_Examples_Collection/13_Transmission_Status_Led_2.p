  ...
  if(iTimerLed > 0)                         // If the counter used to control the LED has not expired ->
  {
    ...
    if(iTimerLed == 0)                      // If the counter used to control the LED has expired ->
    {
      switch(iState)                        // Switches current state that should be displayed via the LED 
      {
        ...
        case STATE_ON:
        {
          Led_On(false, true);              // Makes the LED light up green
        }
        case STATE_SLOW:
        {
          Led_Off(true, true);
          Led_Blink(-1, 1);                 // Makes the LED blink 1x green
        ...
        {
          Led_Off(true, true);
          Led_Flicker(-1, 0);               // Makes the LED flicker green until another Led_xxx function is called
        }
        case STATE_ERROR:
        {
          Led_Off(true, true);
          Led_Blink(2, -1);                 // Makes the LED blink 2x red
