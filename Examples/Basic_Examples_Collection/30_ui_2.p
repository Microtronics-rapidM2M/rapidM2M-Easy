 ...
 * 
 * Reads the last valid values of all four UI channels periodically (record interval) and
...
const
{
  ...
  HISTDATA0_SIZE = UI_NUM_CHANNELS * 4 + 1, // Measurement data block consists of: "Split-tag" (u8) + UI1 (u32) + UI2 (u32) + UI3 (u32) + UI4 (u32)
}
...
/* Reads the last valid values of all four UI channels, compiles the data record to  
...
RecordData()
{
  ...
  /* The for loop gets the measurement values of all four myDatalogEASY V3 channels and prints
     them to the console. Each measurement is written to the "aRecData" array with calculated
     byte offset. An index variable counts up until it reaches the maximum UI channel number
     UI_NUM_CHANNELS */
