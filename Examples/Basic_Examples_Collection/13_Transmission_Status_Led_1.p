 ...
 *
 * Initiates a connection to the server and uses the LED to indicate the current connection state
  ...
  if(iTimerLed > 0)                         // If the counter used to control the LED has not expired ->
  {
    ...
    if(iTimerLed == 0)                      // If the counter used to control the LED has expired ->
    {
      switch(iState)                        // Switches current state that should be displayed via the LED 
      {
        ...
        {
          Led_Off(true,true);
          Led_Blink(0, 0);                  // Makes the LED blink yellow until another Led_xxx function is called 
        }
        case STATE_FAST:
        {
          Led_Off(true, true);
          Led_Flicker(0, 0);                // Makes the LED flicker yellow until another Led_xxx function is called
        }
      }
    }
  }
